...
import static newcloud.ExceuteData.LearningAndInitScheduleTest.brokerId;
 ...
 */
public class PowerDatacenterLearningAndInit extends PowerDatacenter {
    ...
    private VmAllocationAssignerLearningAndInit vmAllocationAssignerLearningAndInit;
    ...
    public PowerDatacenterLearningAndInit(
            ...
            double schedulingInterval, VmAllocationAssignerLearningAndInit vmAllocationAssignerLearningAndInit) throws Exception {
        ...
        setMigrationCount(0);
        this.vmAllocationAssignerLearningAndInit = vmAllocationAssignerLearningAndInit;
        resetEnvironment();
    }
...
//        }
    ...
    public void getReward(Vm vm) {
        ...
        if (everyhosthistorypower.size() >= 2) {
            ...
            for (int i = 0; i < getHostList().size(); i++) {
                if (everyhostcurrentpower.get(i) != 0) {
                    reward = Math.pow(everyhostlastpower.get(i) / everyhostcurrentpower.get(i), 100);
                } else {
                    reward = 0;
                }
                totalreward += reward;
        ...
        vmAllocationAssignerLearningAndInit.createLastState_idx(historycpu);
        vmAllocationAssignerLearningAndInit.createState_idx(currentcpu);
        vmAllocationAssignerLearningAndInit.updateQList(targetHost.getId(), totalreward, historycpu, currentcpu, vm);
    }
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {
        Vm vm = (Vm) ev.getData();
        int hostid = vmAllocationAssignerLearningAndInit.createAction(currentcpu, vm);
